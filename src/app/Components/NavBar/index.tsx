        ...
        <div className="flex justify-center">
            ...
            >
                ...
                <div className="flex items-center">
                    <Link href={"/"}>
                        <Image src="/logo.svg" alt="Youth Ablaze" width={35} height={35} />
                    </Link>
                </div>

                {/* Desktop Navigation */}
                <ul className="hidden md:flex gap-8 ml-[15%] ">
                ...
                {menuOpen && (
                    <div className="absolute top-14 mt-5 left-0 w-full bg-white shadow-lg  py-4 flex flex-col items-center space-y-4 md:hidden rounded-lg">
                        <Link href={"/"} onClick={() => setMenuOpen(false)}> <p className="hover:text-teal-600">Home</p></Link>
                        <Link href={"/aboutus"} onClick={() => setMenuOpen(false)}><p className="hover:text-teal-600">About Us</p></Link>
                        <Link href={"/getinvolved"} onClick={() => setMenuOpen(false)}><p className="hover:text-teal-600">Get Involved</p></Link>
                        <button className="flex items-center gap-1 group align-middle justify-center rounded-full px-4 py-2 hover:bg-gray-200 transition-all duration-300">
                            Join us
                            <FaArrowRight className="transform transition-transform mt-1 duration-300 ease-in-out group-hover:translate-x-1" />
                        </button>
